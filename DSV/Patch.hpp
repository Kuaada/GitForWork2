...
#include <QDebug>
template<>
inline const SlideColorManagement::DataType Patch<unsigned char>::getDataType() const {

    return SlideColorManagement::DataType::UChar;
}

template<>
inline const SlideColorManagement::DataType Patch<float>::getDataType() const {

    return SlideColorManagement::DataType::Float;
}

template<>
inline const SlideColorManagement::DataType Patch<unsigned int>::getDataType() const {

    return SlideColorManagement::DataType::UInt32;
}

template<>
inline const SlideColorManagement::DataType Patch<unsigned short>::getDataType() const {

    return SlideColorManagement::DataType::UInt16;
}

template<>
inline const SlideColorManagement::DataType Patch<double>::getDataType() const {

    return SlideColorManagement::DataType::Float;
}

template<typename T>
const SlideColorManagement::DataType Patch<T>::getDataType() const {
...
template<typename T>
Patch<T>::Patch(const std::vector<unsigned long long>& dimensions, const SlideColorManagement::ColorType& colorType, T* data, bool ownData, std::vector<double> wsiMinValues, std::vector<double> wsiMaxValues) :
    ImageSource(),
    ...
    _ownData(ownData)
{
    ...
    if (!dimensions.empty()) {
        if ((_colorType == SlideColorManagement::ColorType::RGBA && dimensions.back() != 4) || (_colorType == SlideColorManagement::ColorType::RGB && dimensions.back() != 3) || (_colorType == SlideColorManagement::ColorType::Monochrome && dimensions.back() != 1)) {
            _colorType = SlideColorManagement::ColorType::Indexed;
        }
    }
