...
private:
    ...
    /** @brief 性能测量相关成员变量 */
    QElapsedTimer m_loadTimer;           // 加载计时器
    QElapsedTimer m_zoomTimer;           // 缩放计时器  
    QElapsedTimer m_panTimer;            // 平移计时器
    bool m_isFirstLoad = true;           // 是否首次加载
    QList<int> m_fpsHistory;             // FPS历史记录
    int m_fpsHistorySize = 10;           // 保留最近10秒的FPS
...
private slots:
    ...
    void updateFPS() {
        ...
        qDebug() << "📊 FPS:" << m_currentFPS << "| Avg FPS:" << avgFPS;
    }
    /**
    * @brief   直线长度中转
    * @details 更新当前的FPS值并发出信号
